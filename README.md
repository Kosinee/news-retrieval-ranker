...
Проект реализует полный цикл работы **retriever-модели (bi-encoder)**.

**Бизнес-цель:**
- Ускорить поиск релевантной финансовой информации для пользователей и сократить время на нахождение ответа.
...
Полное восстановление версии данных/модели из Google Drive:
```bash
git clone <repo>
...
### Настройка доступа к Google Drive (OAuth client)
Нужно указать client ID/secret в `.dvc/config`:
```
[remote "gdrive"]
...
Если `dvc pull` ругается на отсутствие модуля, установите пакет:
```bash
